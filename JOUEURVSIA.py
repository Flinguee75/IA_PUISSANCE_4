...
def est_libre(plateau, col):
	return plateau[ Nombre_Ligne-1][col] == 0
...
def minimax(plateau, depth, alpha, beta, maximizingJOUEUR):
...
while not game_over:
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.MOUSEMOTION:
			pygame.draw.rect(ecran, BLACK, (0,0, largeur, SQUARESIZE))
			posx = event.pos[0]
			if tour == JOUEUR:
				pygame.draw.circle(ecran, RED, (posx, int(SQUARESIZE/2)), RADIUS)
	...
	if tour == AI and not game_over:				

		#col = random.randint(0, Nombre_Collones-1)
		#col = pick_best_move(plateau, AI_PIECE)
		col, minimax_score = minimax(plateau, 5, -math.inf, math.inf, True)
		print(minimax_score)
		if est_libre(plateau, col):
